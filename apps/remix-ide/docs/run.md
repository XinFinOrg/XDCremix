...
-------------
If you are using Geth & https://remix.xinfin.network, please use the following Geth command to allow requests from Remix:

**geth --rpc --rpccorsdomain https://remix.xinfin.network**

Also see [Geth Docs about the rpc server](https://geth.ethereum.org/docs/rpc/server)

To run Remix using  https://remix.xinfin.network & a local test node, use this Geth command:

**geth --rpc --rpccorsdomain="https://remix.xinfin.network" --rpcapi web3,eth,debug,personal,net --vmdebug --datadir <path/to/local/folder/for/test/chain> --dev console**
